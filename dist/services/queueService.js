...
exports.leaveQueue = exports.getCustomerActiveQueueItem = exports.toggleQueuePause = exports.updateQueueItemStatus = exports.joinWalkIn = exports.joinQueue = exports.getShopQueue = exports.getQueuePreview = exports.calculateCurrentPosition = void 0;
...
const joinQueue = async (shopId, customerId, serviceIds) => {
    ...
    if (status !== shop_1.ShopStatus.ACTIVE) {
        if (status === shop_1.ShopStatus.PAUSED)
            throw new AppError_1.AppError('Queue is currently paused or no shop owner available', 400);
...
const updateQueueItemStatus = async (itemId, status, shopOwnerId, barberId) => {
    return await prisma_1.default.$transaction(async (tx) => {
        ...
        if (status === client_1.QueueStatus.COMPLETED && (shopOwnerId || barberId) && item.customerId) {
...
const toggleQueuePause = async (shopId, isPaused) => {
    return await prisma_1.default.queue.update({
        where: { shopId },
        data: { isPaused }
    });
};
exports.toggleQueuePause = toggleQueuePause;
const getCustomerActiveQueueItem = async (customerId) => {
    ...
    const itemPosition = await (0, exports.calculateCurrentPosition)(item.id);
    ...
    // Get all items ahead
    const queueItemsAhead = await prisma_1.default.queueItem.findMany({
        where: {
            queueId: item.queueId,
            status: { in: [client_1.QueueStatus.WAITING, client_1.QueueStatus.SERVING] },
    ...
    queueItemsAhead.forEach((qi) => {
        const d = qi.services.reduce((acc, s) => acc + s.duration, 0);
    ...
    const fullQueue = await prisma_1.default.queueItem.findMany({
        where: {
            queueId: item.queueId,
            status: { in: [client_1.QueueStatus.WAITING, client_1.QueueStatus.SERVING] }
        },
...
exports.getCustomerActiveQueueItem = getCustomerActiveQueueItem;
